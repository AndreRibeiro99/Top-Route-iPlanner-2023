...
public class SequentialSearchST<Key, Value> implements Serializable  {
    private int n;           // number of key-value pairs -> tamanho de ST
    private Node first;      // the linked list of key-value pairs -> primeiro no da ST
     ...
     * @throws IllegalArgumentException if {@code key} is {@code null}
     */
    public void put(Key key, Value val) { // inserçao á cabeça na lista
        ...
        for (Node x = first; x != null; x = x.next) { // nao pode haver chaves repetidas tem de percorrer ST para ver se encontra chave repetida
            if (key.equals(x.key)) {
                x.val = val;
                return;
            }
